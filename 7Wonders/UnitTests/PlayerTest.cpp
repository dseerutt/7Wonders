...
namespace UnitTests
{
	...
	public:
		...
		TEST_METHOD(PlayerConstructor)
		{
			...
			Assert::AreEqual((unsigned)0, cp.getScore());
			Assert::AreEqual((unsigned int) 0, cp.getMilitaryMalus());
		...
		TEST_METHOD(PrepareTurnETPlayTurnTest)
		{
			...
			//PickACard n'Èlimine pas la carte choisie
			Assert::AreEqual(8, (int)cp.getHand().size());
			cp.playTurn();
			//playTurn Èlimine la carte choisie
		...
		TEST_METHOD(playTurnTestBadCases1)
		{
			...
			try {
				...
				Assert::AreEqual("Pas de carte ÅEjouer sÈlectionnÈe", ex);
			}
		}
		...
		TEST_METHOD(PrepareTurnTestBadCases1)
		{
			...
			try {
				...
				Assert::AreEqual("Un joueur ne possËde aucune carte en main", ex);
			}
		}
